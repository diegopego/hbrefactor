...
> **CONCLUÍDA em 2026-07-07**: Q1-Q8 fechadas com prova executável
> (casos 75-81 + atualizações 64/72-74); critério de pronto atendido.
> O documento fica como registro e régua para trabalho futuro.
...
| Q5 | ✅ **FECHADA (2026-07-07, casos 81 + 71 novo; autorização do Diego: opção B, chamada única)** — o `methodQuery` (regex hbclass) MORREU: a extensão manda a POSIÇÃO do cursor (`usages --at arq:linha:col`, conversão 0→1-based testada no harness) e o CLI resolve por FATO na MESMA invocação; `resolve-at <proj> <arq> <linha> <col>` é o mesmo core standalone (`ResolveAtQuery`). Camadas de fato: nome que preenche marker → dona pelo fecho de derivação DAQUELE site — co-derivação (`PpMarkerOwners`), **aplicação-identidade** (P1a; probe da Q5 provou que o `from` da implementação hbclass deriva das posições da DECLARAÇÃO — a identidade inteira na mesma app é o fato que liga o site) e canal declared sequencial (`_HB_CLASS`/`_HB_MEMBER`, cobre DSL declarativa pura e a lista `{ }` do VAR); palavra de DSL responde a própria; send é dispatch dinâmico → consulta crua honesta; posição vazia recusa e a extensão cai para a palavra crua (degrada, nunca corrompe). Homônimos resolvem pelo SITE, não por unicidade de projeto. Latência do F12 = a de hoje (~1,0s hbhttpd; a alternativa de 2 chamadas custava ~1,9s — medido). Coluna do editor é UTF-16 vs byte do compilador: desalinhamento cai no fallback. Caso 81 (13 checks, todas as camadas + `--at`); caso 71 novo (harness `test-resolveat.js` extrai as funções reais e asserta a morte da regex); extensão 0.6.0. A B4g (`match[]`/`result[]`) completa a cobertura para sites dentro de diretiva. | `ResolveAtQuery` + `usages --at` + extensão por posição; casos 81 e 71 | — |

...
| Q4 | **`ClassParentsSeq` lê pai falso?** (V3) | fixture adversarial: DSL cuja linha de declaração carrega identificador NÃO-pai referenciado na expansão; conferir ClassGraph/dispatch | achar o fato que distingue pai (candidato: a REFERÊNCIA `@Pai()` na árvore de registro em statements[], que é fato de expansão, não posição na linha); sem fato → pais = desconhecido honesto (dispatch degrada para possible, nunca envenena) |

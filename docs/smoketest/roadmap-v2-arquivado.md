...
### Fase 4 — Dogfooding em projeto real ✅ (2026-07-04)

> **Status 2026-07-04**: CONCLUÍDA — ver [dogfooding.md](dogfooding.md).
> Rodada 1 (a ferramenta sobre si mesma): fricção de statements continuados
> por `;` corrigida (`StmtEdits`; caso 24); auto-refatoração `hHit`→`aHit`.
> Rodada 2 (produção, `contrib/hbhttpd` apontado pelo Diego): 7 fricções
> corrigidas — include de sistema via `HB_BIN`/`HB_INC`; erros de compilação
> sempre visíveis (`-q` + filtro); `unused-locals` recusa build quebrado;
> `-inc` ignorado; `${hb_name}` expandida; `.hbc` referenciado contribui
> `incpaths=` (resolução espelha `HBC_Find` do hbmk2); métodos endereçáveis
> por convenção `<Classe>_<Método>` (`usages`, `PickFunc`: `Run`,
> `UHttpd:Run`). Renames/reorder/extract reais com ida-e-volta byte-exata.
> Casos 29-30; `make test` 108/108. Backlog novo: dump v4 (nomes de
> mensagem de `__clsAddMsg` — declaração de METHOD e futuro rename-method).
...
### Fase 5.5 — hbmk2 como resolvedor de projeto ✅ (2026-07-04)

Feedback do Diego virou princípio: **reutilizar o builder oficial,
reescrever só o necessário**. O parser textual de `.hbp`/`.hbc` foi apagado;
`LoadProject` delega a `hbmk2 <alvos> -traceonly` e consome a linha "Harbour
compiler command" (fontes + `-i`/`-D`/`-u+`/`-n` resolvidos). Spec de
projeto = qualquer alvo aceito pelo hbmk2. Junto: modo de **cobertura
parcial** para monólitos (comandos read-only seguem com módulos quebrados;
renames de módulo único exigem só o alvo compilando) e `extract-function`
migrando locais exclusivas da seleção. Casos 29-31; 118/118.

---

## Próximos passos — ordenados por valor de entrega

Regra de ordenação: o que destrava **uso diário em produção** (staff500 +
hbhttpd) vem primeiro; capacidades novas vêm depois, na ordem em que o
código legado mais precisa delas; o upstream fecha porque depende de ordem
explícita do Diego. Documento vivo: fase concluída ganha status aqui antes
da próxima começar (ver regra de manutenção acima).

### Fase 6 — Uso diário viável no monólito (velocidade + ergonomia)

**Por que primeiro**: a rodada 3 provou correção no staff500, mas 21 s por
rename de local (2 × 101 compilações para editar 1 arquivo) não vira hábito.
Valor = a ferramenta entrar na rotina do Diego, que é de onde saem as
próximas fricções reais.
**Escopo**:
- verificação proporcional à edição: rename de módulo único compila
  antes/depois **só o módulo alvo** (+ módulos que o `#include`m, se
  existirem — detectável no `.ppo`); meta ~1-2 s no staff500;
- cache do resolvedor: reusar a saída do `-traceonly` entre comandos na
  mesma sessão de trabalho (invalidação por mtime do `.hbp`/`.hbc`);
- `--include`/documentação da env `INCLUDE` no CLI (a extensão já tem
  `hbrefactor.includePaths`);
- keybindings padrão da extensão (F2/Shift+F12) + empacotamento `.vsix`.
**Critério de pronto**: rename de local no staff500 em ≤2 s; Diego executa
usages/rename do editor sem abrir terminal; suíte verde com caso de tempo
não-regressivo (limite generoso, ex.: fixture 2 módulos ≤3 s).

### Fase 7 — `rename-define` (o rename que o legado mais pede)
...
`#command` tratada com conservadorismo (H). Estudo de caso pronto: a regra
`( <x> & <y> ) => HB_BITAND(...)` do `hbcompat.ch` do staff500, que colide
com o operador de macro — a ferramenta deve **detectar e explicar** colisões
desse tipo.
...
### Fase 8 — Métodos de verdade (dump v4 + rename-method)

**Escopo**: gravar no dump os nomes de mensagem de `__clsAddMsg`/`ADDMETHOD`
(hoje a declaração `METHOD x()` no corpo da classe é invisível — o nome
viaja como string da expansão do hbclass.ch). Com isso: `usages` de método
completo (declaração + definição + sends, sem heurística de sufixo),
`call-graph` com sends, e `rename-method` (definição `<Classe>_<Método>`,
declaração na classe, sends `::x`/`obj:x` — strings continuam relato+
`--force`, nunca edição). hbhttpd e o staff500 (formutil_class etc.) são os
alvos de dogfood.
**Critério**: fixtures de comportamento (execução idêntica) + `usages Stop`
no hbhttpd achando declaração, definição e sends sem rótulo de convenção.

### Fase 9 — `inline-local` + extract v2

**Escopo**: `inline-local` (dual do extract: S quando expressão pura usada
uma vez; H/recusa com chamadas — ordem de efeitos); extract v2 = permitir
`RETURN` quando a seleção é o rabo da função, reindentação do corpo
extraído (hoje mantém o recuo original; `hbformat` cobre no meio-tempo).
**Critério**: fixtures de comportamento (execução idêntica) + recusas.

### Fase 10 — Extensão madura

**Escopo**: preview de edições (`--dry-run --json` → diff virtual), code
action para extract na seleção, progresso/cancelamento para projetos
grandes; o que mais o uso diário da Fase 6 apontar.
**Critério**: Diego usa no dia a dia sem abrir terminal para os fluxos
comuns (reafirmado com o uso real, não por checklist).

### Fase 11 — Upstream do `-x` (bloqueada: só quando o Diego mandar)
...
consumidor real (agora com dois projetos de produção dogfooded).
...
1. `PRIVATE x := init` / `PUBLIC`: declaração e escrita inicial não aparecem (caminho RTVAR não instrumentado) — pré-requisito para qualquer comando sobre memvars. ✅ (v3)
2. Variáveis com alias (`FIELD->x`, `M->x`) — caminho `GenPushAliasedVar`. ✅ (Fase 5)
3. Sends de método e `Eval` em `calls` (hoje otimizados como mensagem, invisíveis) — melhora `usages`/`call-graph` e habilita rename de método no futuro. ✅ (v3, campo `sends`)
4. Dedup das duplicatas de pré/pós-decremento em fallback (cosmético; consumidor já trata por linha) — **não-fazer** justificado na Fase 5.
5. **(novo, da Fase 4 rodada 2) dump v4**: nomes de mensagem de `__clsAddMsg`/`ADDMETHOD` (declaração `METHOD x()` no corpo da classe é invisível — o nome viaja como string); habilita `usages` completo de métodos e um futuro `rename-method`.

**Ferramenta (hbrefactor)**:
5. ✅ `rename-function`: varre `.hbx`/`DYNAMIC` (Fase 5); `REQUEST`/`EXTERNAL` na varredura fora-do-oráculo.
6. `extract-function`: permitir `RETURN` quando a seleção é o rabo da função (hoje recusado) e reindentação do corpo — Fase 9.
...
8. ✅ `usages --json` com coluna real (Fase 5).
9. ✅ projetos sem `.hbp` e fidelidade total ao hbmk2 — resolvido de vez pela delegação ao `hbmk2 -traceonly` (Fase 5.5): macros, filtros de plataforma, `sources=`, `prgflags=`, `.hbc` aninhados.

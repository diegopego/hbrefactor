...
# caso DECLARATIVO (fixture do estado esperado, em vez de grep na saída) -
# contrato e porquê em tests/casedir.sh
. "$HERE/casedir.sh"
...
# exit, o fonte depois byte a byte, e a saída INTEIRA (que é o que um grep
# nunca deu: o que a ferramenta NÃO disse).
run_casedir "$HERE/cases/128-p17-alcance"

# o único check que continua sendo bash: a régua do caso 64 é sobre o FONTE
# DA FERRAMENTA, não sobre o caso
! grep -qiE "\bAvisaLimite\b|\bAvisaCota\b|\bVERSAO_DEMO\b|\bDobro\b" "$HERE/../src/hbrefactor.prg"
check "régua do caso 64: nenhuma palavra do caso 128 na ferramenta" $?

# RÉGUA EXECUTÁVEL da fase (item 5 do critério): todo verbo que anuncia um
# rename tem de chamar SayScope. O experimento provou o rot na prática - eu
# mesmo liguei 2 de 6 verbos, e foi esta régua que apontou os 3 que faltavam.
bash "$HERE/regua-escopo.sh" "$HERE/../src/hbrefactor.prg" > "$HERE/tmp/regua.log" 2>&1
check "régua P17: TODO verbo de rename declara o alcance (SayScope)" $?
[ -s "$HERE/tmp/regua.log" ] && cat "$HERE/tmp/regua.log"

# módulo SEM ramo desligado não ganha aviso nenhum (o portão não vira ruído
# de fundo: 36% dos módulos do core têm compilação condicional)
D=$(freshpos case128b)
LT=$(grep -n 'LOCAL nPasso' "$D/pos.prg" | cut -d: -f1)
CT=$(awk -v n="$LT" 'NR==n { print index($0, "nPasso") }' "$D/pos.prg")
( cd "$D" && "$BIN" rename pos.hbp pos.prg:$LT:$CT nEtapa > rn2.log 2>&1 )
! grep -qi "did not compile\|this configuration only" "$D/rn2.log"

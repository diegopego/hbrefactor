...
| `hbrefactor: Rename Symbol under cursor` | **Rename unificado (F2)**: você dá só a POSIÇÃO; o kind — local, parâmetro, STATIC, memvar, função, método, palavra de DSL ou marker de diretiva — vem do FATO da árvore, não de um comando por-espécie. Pede `--force` quando o nome aparece em strings, e recusa nomeando a exceção se a posição não tem símbolo de compilação. Diretiva que o projeto compila é editada junto — o `.ch` e o módulo mudam no mesmo passo, e o relatório diz quais sítios de regra foram tocados. **É o único comando de rename** — os quatro por-kind foram REMOVIDOS na fase U (fatia 2). |

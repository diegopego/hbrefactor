...
STATIC FUNCTION RenameLocal( aArgs )
   ...
   // tolerant: this command edits exactly one module, so other modules
   // that do not compile (the daily reality of legacy monoliths) only
   // shrink the verification set - the target itself must compile
   cTmp := WorkDir()
   IF ! CompileAll( hProj, cSrcPath, cTmp, "before",, @aFailed )
      RETURN Refuse( "no module of the project compiles - fix build errors first" )
   ENDIF
   IF hb_AScan( aFailed, cSrcPath,,, .T. ) > 0
      RETURN Refuse( "'" + cFile + "' itself does not compile - fix build errors first" )
   ENDIF
   ...
   aAfter := {}
   IF ! CompileAll( hProj, cSrcPath, cTmp, "after",, @aAfter ) .OR. ;
      hb_AScan( aAfter, cSrcPath,,, .T. ) > 0
   ...
   FOR EACH nPos IN hProj[ "files" ]
      IF hb_AScan( aFailed, nPos,,, .T. ) > 0
         LOOP                             // untouched and broken before the edit
      ENDIF
      cOut := FNameBase( nPos )
   ...
   OutStd( "verified: all " + hb_ntos( Len( hProj[ "files" ] ) - Len( aFailed ) ) + ;
           " module(s) byte-identical (-gh -l)" + hb_eol() )
   IF ! Empty( aFailed )
      OutErr( PartialNote( aFailed, hProj ) )
   ENDIF
...
STATIC FUNCTION ExtractFunction( aArgs )
   ...
   FOR EACH hVar IN aVars
      ...
      IF ! hVar[ "param" ] .AND. ! hVar[ "before" ] .AND. ! hVar[ "after" ] .AND. ;
         ( xDecl := DeclNameRemoval( cText, aSrc, hVar ) ) != NIL
         // local used only inside the selection: its declaration migrates
         // to the new function (leaving it behind would trip the project's
         // own -w3/-es2 build: W0003/W0033 on the caller)
         AAdd( aMoved, { hVar[ "declLine" ], xDecl, cWhy } )
         LOOP
      ENDIF
   ...
   FOR nI := 1 TO Len( aMoved )
      OutStd( "  LOCAL " + aMoved[ nI ][ 3 ] + " (line " + hb_ntos( aMoved[ nI ][ 1 ] ) + ;
              ") is used only inside the selection - moved into " + cNewName + hb_eol() )
   NEXT
   IF lDryRun
      OutStd( "dry run - no changes written" + hb_eol() )
      RETURN EXIT_OK
   ...
   // migrate the declarations of selection-only locals (bottom-up: a
   // deleted line shifts everything below it)
   ASort( aMoved,,, {| x, y | x[ 1 ] > y[ 1 ] } )
   FOR nI := 1 TO Len( aMoved )
      cTextNew := EditLine( cTextNew, aMoved[ nI ][ 1 ], aMoved[ nI ][ 2 ], cEol )
   NEXT
   hb_MemoWrit( cSrcPath, cTextNew )
...
// replace one line with new content, or delete it when cNew is empty
...
// a LOCAL declared outside the selection but used only inside it should
// migrate into the extracted function. Returns the declaration line's new
// text ("" = drop the whole line), or NIL when the declaration cannot be
// edited safely (initializer, continuation, comment, shared spelling) -
// the variable then stays a parameter, the conservative fallback.
STATIC FUNCTION DeclNameRemoval( cText, aSrc, hVar )

   LOCAL nLine := hVar[ "declLine" ]
   LOCAL hScan, aHits, cClean, cLine, aWords, nCol, nAfter, nI

   IF nLine <= 0 .OR. nLine > Len( aSrc )
      RETURN NIL
   ENDIF
   hScan  := TokenScan( cText, hVar[ "sym" ] )
   aHits  := hb_HGetDef( hScan[ "hits" ], nLine, {} )
   cClean := hb_HGetDef( hScan[ "clean" ], nLine, "" )
   cLine  := aSrc[ nLine ]

   // plain, single-line LOCAL list without initializers; exactly one
   // occurrence of the name on the line
   aWords := hb_ATokens( AllTrim( cClean ) )
   IF Len( aHits ) != 1 .OR. Empty( aWords ) .OR. ! Upper( aWords[ 1 ] ) == "LOCAL" ;
      .OR. ":=" $ cClean .OR. Right( RTrim( cClean ), 1 ) == ";"
      RETURN NIL
   ENDIF

   // the line declares only this variable: drop it (only when there is no
   // trailing comment that would be lost)
   IF Len( aWords ) == 2 .AND. Upper( aWords[ 2 ] ) == Upper( hVar[ "sym" ] )
      RETURN iif( RTrim( cLine ) == RTrim( cClean ), "", NIL )
   ENDIF

   // remove the name and one adjacent comma from the list
   nCol   := aHits[ 1 ][ 1 ]
   nAfter := nCol + aHits[ 1 ][ 2 ]
   nI := nAfter
   DO WHILE nI <= Len( cLine ) .AND. SubStr( cLine, nI, 1 ) == " "
      nI++
   ENDDO
   IF SubStr( cLine, nI, 1 ) == ","
      nI++
      DO WHILE nI <= Len( cLine ) .AND. SubStr( cLine, nI, 1 ) == " "
         nI++
      ENDDO
      RETURN Left( cLine, nCol - 1 ) + SubStr( cLine, nI )
   ENDIF
   nI := nCol - 1
   DO WHILE nI >= 1 .AND. SubStr( cLine, nI, 1 ) == " "
      nI--
   ENDDO
   IF nI >= 1 .AND. SubStr( cLine, nI, 1 ) == ","
      RETURN Left( cLine, nI - 1 ) + SubStr( cLine, nAfter )
   ENDIF
...
STATIC FUNCTION Usages( aArgs )
   ...
   cTmp := WorkDir()
   IF ! CompileAll( hProj, "", cTmp, "before", .T., @aFailed )
      RETURN Refuse( "no module of the project compiles - fix build errors first" )
   ENDIF

   FOR EACH cPath IN hProj[ "files" ]
      IF hb_AScan( aFailed, cPath,,, .T. ) > 0
         LOOP                       // reported by CompileAll; partial coverage
      ENDIF
      hDump := ReadDump( cTmp + hb_ps() + FNameBase( cPath ) + ".occ.json" )
      IF hDump == NIL
         RETURN Refuse( "missing occurrence dump for '" + cPath + "'" )
      ...
      FOR EACH hFunc IN hDump[ "functions" ]
         ...
         ELSEIF Upper( Right( hFunc[ "name" ], Len( cName ) + 1 ) ) == "_" + Upper( cName )
            // hbclass.ch names method implementations <Class>_<Method>;
            // the method name itself never reaches the dump (it travels as
            // a string argument of __clsAddMsg), so this is a labeled
            // name-convention match, not oracle data
            nHits++
            LocAdd( aLoc, cPath, hFunc[ "line" ], hStrScan )
   ...
   IF ! Empty( aFailed )
      OutErr( PartialNote( aFailed, hProj ) )
      RETURN EXIT_REFUSED             // results shown, but coverage is partial
   ENDIF
...
STATIC FUNCTION RenameStatic( aArgs )
   ...
   LOCAL cText, cTextNew, nI, cPath, aFailed := {}, aAfter
   ...
   // tolerant like rename-local: only the target module is edited
   cTmp := WorkDir()
   IF ! CompileAll( hProj, cSrcPath, cTmp, "before",, @aFailed )
      RETURN Refuse( "no module of the project compiles - fix build errors first" )
   ENDIF
...
STATIC FUNCTION FindDynamicCalls( aArgs )
   ...
   FOR EACH cPath IN hProj[ "files" ]
      ...
      NEXT
      IF hb_AScan( aFailed, cPath,,, .T. ) > 0
         LOOP                        // string scan above needs no dump
      ENDIF
      hDump := ReadDump( cTmp + hb_ps() + FNameBase( cPath ) + ".occ.json" )
      FOR EACH hFunc IN hDump[ "functions" ]
...
STATIC FUNCTION UnusedLocals( aArgs )
   ...
   LOCAL hProj, cPath, cInc, cOut, cErr, cLine, nRes
   LOCAL nFound := 0, aFailed := {}
   ...
   IF hProj == NIL
      RETURN Refuse( "cannot read project file '" + aArgs[ 2 ] + "'" )
   ENDIF
   // the project's own -w/-es levels are dropped: this command's whole
   // point is the -w3 analysis, and a project -es2 would turn it into
   // a build failure instead of a report
   cInc := FlagsStr( hProj, .T. )

   FOR EACH cPath IN hProj[ "files" ]
      cOut := cErr := ""
      nRes := hb_processRun( HarbourBin() + " " + cPath + " -q0 -w3 -s" + cInc,, @cOut, @cErr )
      IF nRes != 0
         // a broken module produces no warnings; keeping quiet here would
         // silently pass code that does not even compile - report it and
         // keep analysing the healthy modules (partial coverage)
         OutErr( "hbrefactor: " + cPath + " does not compile:" + hb_eol() )
         OutErr( CompileErrLines( cOut + cErr ) )
         AAdd( aFailed, cPath )
         LOOP
      ENDIF
...
STATIC FUNCTION PartialNote( aFailed, hProj )
   RETURN "hbrefactor: partial coverage - " + hb_ntos( Len( aFailed ) ) + " of " + ;
          hb_ntos( Len( hProj[ "files" ] ) ) + " module(s) did not compile" + hb_eol()
...
// ---------------------------------------------------------------------------
// project handling - delegated to hbmk2, the official Harbour builder.
// hbmk2 understands .hbp/.hbc (macros, platform filters, nested deps,
// sources=, prgflags=) and its -traceonly prints the exact harbour command
// it would run; that line is our contract: source list + resolved compiler
// flags (-i include paths, -D defines, -u+ auto-included headers, -n mode).
// The project spec is anything hbmk2 accepts as targets: a .hbp, a .hbc
// with sources=, .prg files (comma- or space-separated).
// ---------------------------------------------------------------------------
...
STATIC FUNCTION LoadProject( cSpec )
   ...
   NEXT
   IF Empty( cCmdLine )
      OutErr( "hbrefactor: hbmk2 produced no Harbour compiler command for '" + ;
              cSpec + "'" + hb_eol() )
      RETURN NIL
   ENDIF

   hProj := { "files" => {}, "inc" => {}, "hbx" => {}, "flags" => {} }

   FOR EACH cTok IN CmdTokens( cCmdLine )
      DO CASE
      CASE cTok:__enumIndex() == 1              // the harbour binary itself
      CASE ! Left( cTok, 1 ) == "-"
         DO CASE
      ...
      CASE Left( cTok, 2 ) == "-o" .OR. Left( cTok, 2 ) == "-q"
         // output dir and verbosity are ours to set per invocation
      CASE Left( cTok, 2 ) == "-i"
         AAdd( hProj[ "inc" ], SubStr( cTok, 3 ) )
         AAdd( hProj[ "flags" ], cTok )
      OTHERWISE                                 // -n1, -w3, -es2, -D..., -u+...
         AAdd( hProj[ "flags" ], cTok )
      ENDCASE
   NEXT
   ...
   // the dir of each source participates in #include resolution regardless
   // of the tool's cwd (matters when the spec is a plain .prg list)
   FOR EACH cTok IN hProj[ "files" ]
      ...
      IF hb_AScan( hProj[ "inc" ], cDir,,, .T. ) == 0
         AAdd( hProj[ "inc" ], cDir )
         AAdd( hProj[ "flags" ], "-i" + cDir )
      ENDIF
   NEXT

   RETURN hProj

// hbmk2 quotes tokens with '...' on unix and the traced argv[0] comes as
// ('/path/harbour'); split on blanks outside quotes and strip the wrapping
...
// the compiler flags hbmk2 resolved for this project (-n mode, -i include
// paths, -D defines, -u+ headers, -w/-es levels), space-joined; lNoWarn
// drops the project's -w/-es so a command can impose its own levels
STATIC FUNCTION FlagsStr( hProj, lNoWarn )

   LOCAL cTok, cFlags := ""

   hb_default( @lNoWarn, .F. )

   FOR EACH cTok IN hProj[ "flags" ]
      IF lNoWarn .AND. ( Left( cTok, 2 ) == "-w" .OR. Left( cTok, 3 ) == "-es" )
         LOOP
      ENDIF
      cFlags += " " + cTok
   NEXT
...
// compile every module of the project (-gh -l hrb for byte comparison;
// dump+ppo for the target or all). With aFailed (by reference) a module
// that does not compile is recorded and the others still proceed - the
// partial-coverage mode of the read-only commands, made for legacy
// monoliths where some module is always broken; without it the first
// failure aborts (write commands must see the whole project).
STATIC FUNCTION CompileAll( hProj, cTarget, cTmp, cTag, lDumpAll, aFailed )

   LOCAL cPath, cCmd, cInc, cOut, cErr, nRes
   LOCAL lTolerant := HB_ISARRAY( aFailed )
   ...
   FOR EACH cPath IN hProj[ "files" ]
      // -q (not -q2): -q2 also swallows error messages, leaving nothing
      // to report when a real-world project fails to compile
      cCmd := HarbourBin() + " " + cPath + " -q -gh -l" + cInc + ;
   ...
   RETURN ! lTolerant .OR. Len( aFailed ) < Len( hProj[ "files" ] )

// keep only the meaningful lines (errors/warnings) of a compiler run,
// dropping the banner that -q still prints
STATIC FUNCTION CompileErrLines( cText )

   LOCAL cLine, cRes := ""

   FOR EACH cLine IN hb_ATokens( StrTran( cText, Chr( 13 ), "" ), Chr( 10 ) )
      IF " Error " $ cLine .OR. " Warning " $ cLine
...
STATIC FUNCTION PickFunc( hDump, cFunc )
   ...
   // methods are addressable as Class:Method or by their hbclass.ch
   // implementation name <Class>_<Method>
   ...
   // bare method name: unique name-convention suffix match
   FOR EACH hFunc IN hDump[ "functions" ]
